...
void sum(int start, int end){
    ...
    if(start%10000==0&&start+9999<=end){
        ...
        sum(start+10000, end);
        return;
    }else if(start%1000==0&&start+999<=end){
        one1+=check[3][start].o;
        two1+=check[3][start].t;
        three1+=check[3][start].thr;
        sum(start+1000, end);
        return;
        ...
        sum(start+100, end);
        return;
    }else if(start%10==0&&start+9<=end){
        one1+=check[1][start].o;
        two1+=check[1][start].t;
        three1+=check[1][start].thr;
        sum(start+10, end);
        return;
    }else{
