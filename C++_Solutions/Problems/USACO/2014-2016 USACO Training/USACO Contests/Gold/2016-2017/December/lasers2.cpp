...
int main(){
    ...
    while(!pq.empty()){
        ...
        if(XtoY.find(curr.x)!=XtoY.end() && curr.v == true){
            for(int i = 0; i < XtoY[curr.x].size(); i++){
                pii pos = make_pair(curr.x, XtoY[curr.x][i]);
        ...
        }else if(YtoX.find(curr.y)!=YtoX.end() && curr.v == false){
            for(int i = 0; i < YtoX[curr.y].size(); i++){
                    pii pos = make_pair(YtoX[curr.y][i], curr.y);
                    if(visited.find(pos)!=visited.end()){
                        continue;
                    }
