    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if insect.is_queen is True:
                    return
                """elif insect.is_queen is False: #me writing extra check cases :(
                    if self.ant is None:
                        return
                    elif self.ant is not None:
                        if hasattr(self.ant.ant, 'container'):
                            self.ant  = container
                        elif self.ant.ant is not None and self.ant.ant """
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        ...
        if self.container is True:
            if self.ant = None and other.container = False:
                return True
        return False
        """
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        #print(new_place)
        while new_place != hive and new_place!=None:
            #print(new_place.entrance, hive)
            if new_place.bees!=[]:
                #print("hello")
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        counter = 0;
        while new_place != hive and new_place!=None:
            if new_place.bees != [] and counter >= self.min_range:
                return random_or_none(new_place.bees)
            else: 
                new_place = new_place.entrance
            counter = counter+1
...
class ShortThrower(ThrowerAnt):
    ...
    implemented = True
    food_cost = 2 
    max_range = 3 # Change to True to view in the GUI
    # END Problem 4B
    def nearest_bee(self, hive):
        new_place = self.place
        counter = 0;
        while new_place != hive and new_place!=None:
            if new_place.bees!=[] and counter <= self.max_range:
                return random_or_none(new_place.bees)
            elif counter > self.max_range:
                return None
            else:
                new_place = new_place.entrance
            counter = counter+1
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        "*** REPLACE THIS LINE ***"
        if self.digesting == 0:
            if random_or_none(self.place.bees) is not None:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 9
        "*** REPLACE THIS LINE ***"
        if self.is_queen is False:
            self.reduce_armor(self.armor)
        else:
            ScubaThrower.action(self, colony)        
            ant_behind = self.place.exit
            
            # If the current place is a container double the containers damage
            if self.place.ant.container is True:
                self.place.ant.damage*=2
            # while the place is not None continue doubling the damage of all ants including containers
            while ant_behind is not None:
                #print(self.ants_doubled)
                #print(ant_behind, ant_behind)
                if ant_behind.ant is not None and ant_behind.ant not in self.ants_doubled: #check if the ant is not doubled or none
                    if ant_behind.ant.container is True: #check if the ant is container
                        ant_behind.ant.damage *= 2 #double ant containers damage
                        if ant_behind.ant.ant is not None and ant_behind.ant.ant not in self.ants_doubled: #check if the contained ant is doubled
                            ant_behind.ant.ant.damage *= 2 #double contained ant
                            self.ants_doubled.append(ant_behind.ant.ant) #add the contained ant
                    elif ant_behind.ant.container is False: #no container
                        ant_behind.ant.damage *= 2 #double ant damage
                    self.ants_doubled.append(ant_behind.ant) #append ant to doubled ant damage
                elif ant_behind.ant is not None and ant_behind.ant in self.ants_doubled: #account for edge case when container is doubled already but not ant inside
                    if ant_behind.ant.container is True:
                        if ant_behind.ant.ant is not None and ant_behind.ant.ant not in self.ants_doubled:
                            ant_behind.ant.ant.damage *= 2
                            self.ants_doubled.append(ant_behind.ant.ant)
                ant_behind = ant_behind.exit
        # END Problem 9
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.is_queen is True:
            bees_win()
        elif self.armor <= 0 and self.is_queen is False:
            self.place.remove_insect(self)
        # END Problem 9
...
def make_slow(action, duration):
    ...
    def new_action(colony):
        ...
        counter = counter + 1
        #print(counter)
        if counter <= duration and colony.time % 2 == 1:
            #print(colony.time, duration, counter)
            return None
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** REPLACE THIS LINE ***"
    #print(effect, bee.action, bee, duration)
