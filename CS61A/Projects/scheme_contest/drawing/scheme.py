...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        operator = scheme_eval(first, env) #evaluate operator
        check_procedure(operator)
        operands = rest.map(lambda operand: scheme_eval(operand, env)) #evaluate operands
        return scheme_apply(operator, operands, env) #apply operator to operands
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals.__len__() != vals.__len__():
            raise SchemeError
        while formals != nil:
            first, second = formals.first, vals.first
            child.define(first, second)
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            return self.fn(*python_args) #apply the function to each element in python_args
        except TypeError:
            raise SchemeError
        # END PROBLEM 4
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        call_frame = self.env.make_child_frame(self.formals, args)
        return call_frame
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target, value) # binds symbol to value in the current frame
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        function = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, function)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formals = Pair(clause.first, formals)
        vals = Pair(scheme_eval(clause.second.first, env), vals)
        bindings = bindings.second
    check_formals(formals)
    let_frame = env.make_child_frame(formals, vals)
    return let_frame
    # END PROBLEM 15
...
class MuProcedure(UserDefinedProcedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        child_frame = env.make_child_frame(self.formals, args) # make a local frame with env as parent
        return child_frame
    # END PROBLEM 16
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    return MuProcedure(formals, expressions.second) # call instance of MuProcedure
    # END PROBLEM 16
