...
def make_to_string(front, mid, back, empty_repr):
    ...
    def linked(lst):
        ...
        if lst == Link.empty:
            ...
            return front + str(lst.first) + mid + linked(lst.rest) + back
...
def add_trees(t1, t2):
    ...
    #if t1.is_leaf():
    #    return Tree(root, t2.branches)
    #elif t2.is_leaf():
    #    return Tree(root, t1.branches)
    #else:
    
    branches = [add_trees(x, y) for x, y in zip(t1.branches, t2.branches)]
    
    if len(t1.branches) < len(t2.branches):
        for i in range(len(t1.branches), len(t2.branches)):
            branches.append(t2.branches[i])
    elif len(t1.branches) > len(t2.branches):
        for i in range(len(t2.branches), len(t1.branches)):
            branches.append(t1.branches[i])
