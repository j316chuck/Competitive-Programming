...
class Player(object):
    ...
    def go_to(self, location):
        ...
        if destination_place.locked:
            ...
            print("You are at " + destination_place.name)
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            print("Player takes the {:s}".format(thing))
        else:
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
                #print(self.place.get_neighbor(place).name)
                if self.place.get_neighbor(place).locked == True:
...
sather_gate = Place('Sather Gate', 'You are at Sather Gate', [john], [])
me = Player('player', sather_gate)
print(me.talk_to('John'))
print(me.talk_to('Albert'))"""
"""key = Key('SkeletonKey', 'A Key to unlock all doors.')
gbc = Place('GBC', 'You are at Golden Bear Cafe', [], [key])
fsm = Place('FSM', 'Home of the nectar of the gods', [], [])
gbc.add_exits([fsm])
